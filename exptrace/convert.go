...
import (
	...
	"honnef.co/go/gotraceui/exptrace/oldtrace"
	"io"
)
...
func (it *convertIter) init(pr oldtrace.Trace) error {
	it.trace = pr
	it.preInit = true
...
func (it *convertIter) next() (Event, error) {
	if len(it.extra) > 0 {
		ev := it.extra[0]
		it.extra = it.extra[1:]
	...
	ev, err := it.convertEvent(&it.events.Buckets[it.bucket][it.intraBucket])

	it.intraBucket++
	if it.intraBucket == oldtrace.BucketSize || (it.bucket*oldtrace.BucketSize+it.intraBucket) >= it.events.Len() {
...
func (it *convertIter) convertEvent(ev *oldtrace.Event) (OUT Event, ERR error) {
	...
	case oldtrace.EvGoSysCall:
		...
		// blocked.
		blocked := false
		for i := it.bucket*oldtrace.BucketSize + it.intraBucket + 1; i < it.events.Len(); i++ {
			nev := it.events.Ptr(i)
			if nev.G != ev.G {
				continue
			}
...
func convertOldFormat(pr oldtrace.Trace) *convertIter {
	it := &convertIter{}
	it.init(pr)
	return it
}

...
import (
	...
	"errors"
	"fmt"
	"honnef.co/go/gotraceui/exptrace/internal/event"
...
func Parse(r io.Reader, vers version.Version) (Trace, error) {
	// We accept the version as an argument because honnef.co/go/gotraceui/exptrace will have
...
// parseRest reads per-P event batches and merges them into a single, consistent
...
// indexAndPartiallyParse records the offsets of batches and parses strings and
// CPU samples.
...
func (p *Parser) readRawEvent(flags uint, ev *rawEvent) error {
	...
	case EvString:
		if flags&skipStrings != 0 {
			...
			// There aren't enough strings in a trace to justify optimizing away
			// this allocation.
			buf := make([]byte, ln)
			if !p.readFull(buf) {
...
func (p *Parser) readStr() (s string, err error) {
	...
	// There aren't enough strings in a trace to justify optimizing away this
	// allocation, especially because this function is only used for a subset of
	// all strings.
	buf := make([]byte, sz)
	if !p.readFull(buf) {
		return "", fmt.Errorf("failed to read trace: %w", io.ErrUnexpectedEOF)
...
func (p *Parser) postProcessTrace(events BucketSlice) error {
	...
	for evIdx := 0; evIdx < events.Len(); evIdx++ {
		...
		// Note: each branch is responsible for retrieving P and G descriptions
		// and writing back modifications to the maps. Deduplicating this step
		// and pulling it outside the switch is too expensive.
		switch ev.Type {
		case EvProcStart:
			p := ps[ev.P]
			if p.running {

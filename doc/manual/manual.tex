...
% TODO: add nix derivation for building the manual, take inspiration from https://github.com/NixOS/nixpkgs/issues/100086#issuecomment-709567631
...
% - main menu
% - timelines
%   - clicking on label (both to open goroutine panel and to zoom)
%   - spans (zoom, hover, context menus)
% - goroutine panel
%   - it can pop out!
% - user regions
...
Traces also contains information about the work of the garbage collector (\textsc{gc}), indicating its various phases, and how much \textsc{cpu} time goroutines have to contribute to assisting the \textsc{gc}.
...
Alternatively, the context menu of the axis allows quickly placing it at the beginning, middle, or end of the axis.
...
For example, a goroutine may be blocked on a channel send operation for \qty{100}{ms}, and this would be displayed as a single span.
...
Pressing \keys{\shortcut+LMB} on a span will zoom the canvas to the span.
...
By default, samples occur at a frequency of \qty{100}{Hz}, i.e., once every \qty{10}{ms}.
This means that there is \qty{10}{ms} of uncertainty after a sample.
The stack trace might've changed anywhere from \num{0} to \qty{10}{ms} after the sample.
...
You shouldn't rely on sampled stack traces to fill in the gaps between two runtime events that happened \qty{100}{us} apart.
...
If a sample is followed by a runtime event \qty{1}{ms} later then it will look much smaller than if it were followed by a runtime event \qty{9}{ms} later,
even though in the latter case we still don't know what happened for the first \qty{9}{ms}. 
...
The size of a bucket can be adjusted using the arrow keys. The \keys{\arrowkey{<}} and \keys{\arrowkey{>}} keys decrease and increase the amount of time represented by a bucket in steps of \qty{10}{ms}.
...
\begin{tabular}{ll}
  Input & Function \\
  \hline
  \keys{\arrowkey{<}} & Decrease X-axis bucket size by \qty{10}{ms} \\
  \keys{\arrowkey{>}} & Increase X-axis bucket size by \qty{10}{ms} \\
...
For short-lived applications, \code{Start} is best called as early in your main function as possible.
It is also crucial that \code{Stop} gets called, as otherwise an incomplete trace may get written, which may not be parseable at all.
...
Regions can also be nested, for additional detail. They can be started with \code{StartRegion} and stopped with \code{(*Region).Stop}.
...
You can additionally use the \code{-cpuprofile} flag to include CPU profiling samples in the trace.
...
The environment variable \code{GOMAXPROCS} controls the number of available processors.

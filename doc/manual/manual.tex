...
Timestamps and references to goroutines and processors are formatted as clickable links.
Clicking on a timestamp scrolls the canvas to that point in time.
Clicking on a processor or goroutine scrolls the canvas to the corresponding timeline.
Holding \keys{\shortcut} while clicking on a processor or goroutine zooms to it.
Holding \keys{\shift} while clicking on a processor or goroutine opens a processor or goroutine panel, respectively.
Right-clicking on a processor or goroutine opens a context menu with all navigation options.
...
Clicking the \menu{Copy as CSV} button copies the statistics to the clipboard, in \textsc{csv} format.
...
Furthermore, for merged spans, a list of all individual spans is shown.

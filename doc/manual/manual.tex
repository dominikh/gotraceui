...
\definecolor{stateStack][x=79B579]
\definecolor[stateSample][x=98D597]
...
\startmakeup[middle]
  \midaligned{{\externalfigure[images/logo.png][width=0.50\textwidth]}}
  Gotraceui
\stopmakeup
...
\startchapter[title=Introduction]
  ...
  It can display runtime events, CPU profiling information, memory usage, and more.
  Traces can be viewed at nanosecond resolution or at macro scale.
  Millions of events in a single trace are no problem.
\stopchapter
...
\startchapter[title=System requirements]
  ...
  For reference, an example \unit{300 MB} trace file was produced by tracing a busy Prometheus instance for one minute,
  resulting in \unit{66,044,021} events.
...
\startchapter[title=The user interface]
  ...
  \startsection[title=Main window]
    ...
    \startsubsection[title={Timelines, tracks, and spans}]
      ...
      Pressing \key{LMB} on a goroutine label will open a panel with additional information about the goroutine.
      ...
      while the other NUM tracks represent the NUM levels of call stacks acquired from events and CPU samples.
      ...
      but also additional information such as tags \intodo{(link to the section on tags}) or the reason for being in a certain state.
      Pressing \key{LMB} on a span will open a list of events that happened during that span.
      ...
      Such merged spans get their own color \intodo{(which?)}.
      Zooming into merged spans unmerges them.
      Second, if there aren't enough spans to merge, the span is given a minimum size.
      \intodo{mention how that means following spans might be rendered smaller than they are,
...
\startchapter[title=Understanding the Go runtime]
  ...
  \startsection[title=Garbage collection,reference=gc]
    ...
    and you're encouraged to read the \goto{official documentation}[url(https://go.dev/doc/gc-guide)] to learn more about it.

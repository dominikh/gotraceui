...
\startchapter[title=The user interface]
  ...
  \startsection[title=Main window]
    ...
    The timeline can be moved around by dragging with \key{LMB} or by scrolling.
    Holding \key{Ctrl/Cmd} while scrolling zooms in and out of the timeline, centered around the cursor's position.
    Dragging with \key{Ctrl/Cmd}+\key{LMB} selects a region to zoom to.
    ...
    \startsubsection[title={Activities, tracks, and spans}]
      ...
      Pressing \key{Ctrl/Cmd}+\key{LMB} on a label will zoom the timeline such that all spans in that activity are visible.
      ...
      Pressing \key{Ctrl/Cmd}+\key{LMB} on a span will zoom the timeline such that the span is fully visible.

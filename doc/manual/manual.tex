...
\startchapter[title=System requirements]
  ...
  However, due to its current input scheme, it heavily relies on a three button mouse and may thus not be enjoyable to use for most macOS users or users of trackpads.
  ...
  That is, a 300 \unit{MB} trace file will need about 9 \unit{GB} of memory to be loaded by Gotraceui.
  For reference, an example 300 \unit{MB} trace file was produced by tracing a busy Prometheus instance for one minute, resulting in 66,044,021 events.
  This represents an extreme example.
  Many of your traces will be much smaller than that.
  For example, tracing \code{net/http}'s tests produces a 7.3 \unit{MB} trace instead, 
\stopchapter


\startchapter[title=The user interface]
  \startsection[title=Main window]
    The main window displays a main menu and an interactive \noun{timeline} representing the trace data.

    \midaligned{{\externalfigure[images/screenshots/main_window.png][width=0.95\textwidth]}}

    The timeline can be moved around by holding \key{MMB} or \key{Spacebar} while moving the mouse.
    Using the scroll wheel zooms in and out of the timeline, centered around the cursor's position.
    Dragging with \key{Shift}+\key{MMB} selects a region to zoom to.
    The \menu{Display} menu contains commands for changing the way the timeline is displayed, as well as commands for quick navigation.

    \startsubsection[title=Axis]
      The top of the timeline shows the \noun{axis}.
      ...
      For example, the screenshot shows an axis that starts at 2,626,835,792 \unit{ns} and where each major tick represents an increase of 1 \unit{ms}.
      ...
      Pressing \key{O} cycles through displaying the red section, both sections, or none of the sections across the entire timeline.
    \stopsubsection

    \startsubsection[title={Activities, tracks, and spans}]
      The timeline consists of a number of horizontally stacked \noun{activities}.
      An activity might be a processor, a goroutine, or phases of the garbage collector.
      Every activity has a label, hovering over which may display a tooltip.
      ...
      Pressing \key{Ctrl}+\key{MMB} on a label will zoom the timeline such that all spans in that activity are visible.
      ...
      An activity consists of one or more horizontally stacked \noun{tracks}.

      \midaligned{{\externalfigure[images/screenshots/activity.png][width=0.95\textwidth]}}

      For example, the screenshot shows a goroutine activity with six tracks.
      ...
      A span represents an activity's state for some duration of time.
      For example, a goroutine may be blocked on a channel send operation for 100 \unit{ms}, and this would be displayed as a single span.
      ...
      Pressing \key{Ctrl}+\key{MMB} on a span will zoom the timeline such that the span is fully visible.

      Spans have different colors depending on the states they represent.
      Different kinds of activities and tracks may use different color schemes.
...
\startchapter[title=Goroutine states]
  \starttodo
